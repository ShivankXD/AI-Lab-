...
whatis = input("Enter TRUE for Maximization Problem and FALSE for 
Minimization").upper() 
...
openList.append([first,None,heuristics[first]]) 
print(f"Heuristic Values are {heuristics}") 
    ...
    if TestNode[0 in goal: 
        ...
        for children in graph[TestNode[0]: 
            if not is_in_both(children,closeList) and not is_in_both(children,openList): 
                openList.append([children,TestNode[0],heuristics[children]]) 
        return GoalTest() 
 
GoalTest() 
print("Traversal Path:","→".join(list_traversal for list_traversal,_,_ in closeList))
